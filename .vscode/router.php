...
if ($_SERVER['REQUEST_URI'] === '/favicon.ico') {
    return false; // deixa o servidor tratar ou ignora
}
            ...
            <script type="module">
                window.addEventListener('DOMContentLoaded', (event) => {
                    const socket = io(`ws://localhost:5003`, {
                        auth: {
                            token: "ZGY2NDkzMGUtYjczZC00MzkxLTlkZGEtNTExMTZjZGI3MmJiMTc0NjcxMTM4NzIyNg==aMc"
                        },
                        withCredentials: true,
                        reconnectionAttempts: 3
                    });
                    socket.on("connect", () => {
                        console.log(`Connect_success : _${ socket.id }_live _php.`);
                    });
